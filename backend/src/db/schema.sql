...
CREATE TABLE `products` (
  ...
  `gst_rate` DECIMAL(5,2) NOT NULL DEFAULT 0.00,
  `cess_rate` DECIMAL(5,2) DEFAULT NULL COMMENT 'Compensation cess % (NULL = inherit from HSN/SAC master)',
...
CREATE TABLE `invoices` (
  ...
  `original_invoice_id` INT UNSIGNED DEFAULT NULL COMMENT 'CN/DN: the tax invoice this credit/debit note adjusts',
  `against_invoice_no` VARCHAR(60) DEFAULT NULL COMMENT 'Denormalized original invoice number for print/GSTR-1',
...
INSERT INTO `accounts` (`code`,`name`,`type`) VALUES
  ...
  ('2350','GST Output (Cess Payable)','LIABILITY'),
